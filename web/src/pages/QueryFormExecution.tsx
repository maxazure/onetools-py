...
const QueryFormExecution: React.FC = () => {
  ...
  // 模拟服务器列表（实际应该从配置服务获取）
  const serverOptions = [
    { name: 'localhost', label: 'Local Server' },
    { name: 'dev-server', label: 'Development Server' },
    { name: 'test-server', label: 'Test Server' },
  ];
  ...
  const loadForm = async () => {
    try {
      ...
      if (result.success && result.data) {
        setForm(result.data);
        if (result.data.target_database) {
          // 如果有目标数据库配置，设置默认服务器
          setServerName(serverOptions[0]?.name || '');
        }
      } else {
  ...
  return (
    <div className="query-form-execution">
          ...
          <Col>
            <Space>
              ...
              <Button
                ...
                onClick={() => navigate(`/query-forms/${form.id}/edit`)}
              >
                编辑表单
              </Button>
      ...
      {!serverName && (
        <Alert
          message="请选择数据库服务器"
          description="执行查询前请先选择要连接的数据库服务器。"
          type="info"
          showIcon
          style={{ marginBottom: 16 }}
        />
